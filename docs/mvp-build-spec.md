...
3. Borrower Agent posts one `10 XLM` Loan Request with the default `FeeModel`, a `purpose_hash`, and `PrivacyMode { hide_purpose: true, require_eligibility_attestation: false }` for the first run.
...
The first run uses `min_reputation_score = 0` to avoid blocking a fresh testnet borrower. After the happy path works once, enable `require_eligibility_attestation = true` and `min_reputation_score = 50` for the privacy track run.
...
- `purpose_hash`, `PrivacyMode`, and `EligibilityAttestation` references.
...
- signed Eligibility Attestation statements and signature verification;
