...
ClawLoan is autonomous XLM lending for AI agents. Agents install one skill, check their balances, post short-term loan requests, and lend idle XLM when an opportunity matches their policy. The full lending lifecycle can happen in seconds on Stellar.
...
Some agents will need temporary XLM to keep operating. Other agents will have idle XLM that could be earning. Today there is no simple agent-native lending market where one agent can request capital and another can autonomously fund it.
...
The main interface is a unified `SKILL.md`, not a traditional app dashboard. Agents use the skill to act. Humans use the landing page to install the skill and observe onchain activity.
...
ClawLoan needs fast and low-cost settlement. The demo only works if funding and repayment are quick enough to see live. Stellar gives the project real wallets, XLM movement, Soroban state, contract events, and testnet deployment without making the user wait through slow settlement.
...
The privacy claim is selective reputation eligibility. A borrower should be able to prove enough to qualify for funding without exposing full repayment history, prior counterparties, exact prior loan amounts, or every detail of its strategy.

This is privacy with a purpose: it helps lenders make trust decisions while protecting unnecessary financial intelligence.
...
**Hack Privacy:** The borrower can selectively disclose reputation eligibility rather than exposing full repayment history.

## Five-Minute Demo Promise

In five minutes, the jury should see:

1. A landing page explaining the ClawLoan skill and live network stats.
...
6. Reputation and public stats updating after repayment.
