...
Scan `~/.Codex/skills/` for: (a) skill folders with `SKILL.md`, (b) flat `.md` skill files (the SCF skills ship as flat files). Group them into journey phases:
...
Load `~/.Codex/skills/SKILL_ROUTER.md` if present — that's the canonical trigger-phrase → skill mapping.
...
- Don't invent skill names — only use what's in `~/.Codex/skills/`
