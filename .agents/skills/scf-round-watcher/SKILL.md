...
The site is a client-rendered React app — plain `curl` returns mostly empty HTML. Use the **WebFetch tool** (Codex built-in) which handles client-rendered pages via its parsing layer:
...
Load `~/.Codex/skills/data/lumenloop/projects.json` (or fetch from `https://raw.githubusercontent.com/lumenloop/stellar-ecosystem-db/main/...` if missing).
